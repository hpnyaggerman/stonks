...
- **Selection metric** (`best_metric`): default = held-out retrieval accuracy (the acceptance metric). Two optional modes, both computed over the same stratified window set — `strat_eval_windows` (32) windows spread across the sampler's M strata, the full timeline, mirroring how training samples windows — as opposed to the newest-`eval_windows` (16) block that feeds the always-on acceptance metrics:
  - `consistency` — held-out consistency median. Smoother than retrieval's 10-trial granularity (±0.1 steps, std ≈ ±0.13), but scale-dependent and collapse-blind.
  - `margin` — median over holdouts of the **margin ratio** `ρ_h = d(q_h, nearest impostor) / d(q_h, own key)`, the continuous form of retrieval: `ρ > 1` iff a top-1 hit, and the magnitude keeps what the binary hit discards (a ρ = 0.95 near-miss ≠ a ρ = 0.1 total miss). Scale-free — collapse drives both distances to zero and reads as ρ ≈ 1, never as a win — so it carries persistence (denominator) and distinctiveness (numerator) in one smooth, selection-grade number. Logged always as `margin_ratio` (newest-16) and, when a stratified mode is active, `margin_ratio_stratified`.

  In every mode the retrieval column remains the acceptance read.
