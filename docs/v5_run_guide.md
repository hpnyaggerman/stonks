...
Runs live side by side under `models/v5/runs/` and are never overwritten: the launch claims the next numbered `models/v5/runs/rN` (number gaps mark discarded runs) and passes it to every process; `--run-dir` names a run explicitly instead. The launcher tees its own stream into `<run dir>/run.log`, so a detached launch (`screen -dmS v5run bash run_multi_gpu.sh ...`) needs no external redirect and the run dir is self-contained: config, manifest, split meta, per-member history tables, checkpoints, and logs together.

One member per GPU (4 total), batch 256, lr 3e-4, warmup 1000, patience 20 evals, `--val-subsample 150000`, scheduled eval cadence (every 250 steps to 2k, 1000 to 30k, 5000 after). The script exports one `RUN_NONCE` per launch; the member-0 process writes the run dir's `run_manifest.json` (args, git revision, data fingerprint, environment -- library versions, GPU, hostname -- and scaler rows) and the other trainers refuse to run against a stale or mismatched manifest. Before step 1 the member-0 process also writes the launch artifacts: `config.json` (full resolved `V5Config`), `v5_norm.json` (the training scaler), and `split_meta.json` (cut dates, embargo, holdout ticker list, stop/gate block dates, resolved schedule incl. member seeds, window/row counts, baselines, comparator ICs) -- so a killed run stays fully interpretable and scoreable. During training each member maintains four parquet history tables -- `evals_member{i}.parquet` (one row per eval, scalar metrics + LR-gate fields), `ic_member{i}.parquet` (long IC records: step x tag x horizon x score x mask x role x trad), `top_member{i}.parquet` (top-of-ranking records), `train_member{i}.parquet` (per `--log-every` step: lr, loss EMAs, throughput) -- each rewritten via temp file + atomic rename at every tick, so the on-disk file is a complete, readable snapshot at any kill instant (a kill loses at most the in-flight tick; column-level reference: `docs/v5_history_schema.md`); `member_{i}_latest.pt` is written at every eval plus `member_{i}_best.pt` on improvement. After all members finish the script deletes the duplicate per-GPU caches, reuses `cache/v5_gpu0`, and finalizes; the finalize refuses if the data, git revision, or any split-relevant arg changed since training, and asserts its own scaler refit and config equal the launch-written `v5_norm.json`/`config.json`.
...
Trainer (`run_train_v5.py`): 1m/6m train labels are thinned (strides 4/25) to slow the long-horizon memorization clock; early stopping now runs on the smoothed rank-IC stopping score (t_1d + t_1w)/sqrt(2) over stopping blocks of the val year, with a calibrated-CE bootstrap while the score is degenerate; evals follow a step schedule dense inside warmup; every eval appends the full metric suite (both scores, block roles, tradability and uncensored variants, train-panel ICs) to a per-member history file, with gating-side values never printed or consulted; the judged CE, its baseline, and the temperature fit all live on one row set (tag-0 {1d, 1w} stopping-side labels under interval membership); `eval_mode=both` now routes eval tickers' val rows into the val fold (tag 1) and non-eval OOS rows onto the forecast surface (full universe); the class-rate probe measures the actually-trained population; sub-$1 anchors never train; `lam_cls` lives on the config; per-horizon train-CE EMAs print in the step log; the plateau-LR drop is gated on train-loss decline plus sustained stopping-side calibrated-CE regression (the last `--lr-patience` judged CEs all more than `--lr-ce-margin` above the running best), so score-detector noise alone cannot reduce the LR of a learning run.

Provenance: every run writes `run_manifest.json` (args, git revision, data fingerprint incl. census sha256, environment, scaler rows) plus launch-time `config.json`/`v5_norm.json`/`split_meta.json`; multi-GPU launches are nonce-checked; `--forecast-only` refuses on any data/revision/arg drift and asserts scaler/config equality against the launch artifacts. `--run-dir` isolates concurrent undertakings; `--data-start/--data-end` enable rolling origins and the survivorship ablation; `--label-shuffle-within-date` is the end-to-end leakage null; a disk preflight replaces the mid-write Bus error.

Run handling (stock-identity convention): runs are numbered side-by-side dirs under `models/v5/runs/` claimed at launch (`rN`, or a protocol name via `--run-dir`), each self-contained -- `run_manifest.json` (incl. environment), launch-time `config.json` + `v5_norm.json` + `split_meta.json`, per-member `{evals,ic,top,train}_member{i}.parquet` history tables (atomically rewritten every tick; valid at any kill instant), `member_{i}_latest.pt` every eval + `member_{i}_best.pt` on improvement + final `member_{i}.pt`, and the launch/trainer/finalize logs. Consumers read only the promoted `models/v5` (`tools/promote_run.py`, stamped by `promoted_from.json`).
